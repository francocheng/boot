...
/**
 * Created by franco.cheng on 2017/3/29.
 */
public class MapUtil {
    ...
    public static Map<String,String> orderJsonNodeMap(Map<String, JsonNode> map) {
        HashMap<String, String> tempMap = new LinkedHashMap<String, String>();
        List<Map.Entry<String, JsonNode>> infoIds = new ArrayList<Map.Entry<String, JsonNode>>(	map.entrySet());

        Collections.sort(infoIds, new Comparator<Map.Entry<String, JsonNode>>() {
            @Override
            public int compare(Map.Entry<String, JsonNode> o1, Map.Entry<String, JsonNode> o2) {
                return (o1.getKey()).toString().compareTo(o2.getKey());
            }
        });

        for (int i = 0; i < infoIds.size(); i++) {
            Map.Entry<String, JsonNode> item = infoIds.get(i);
            tempMap.put(item.getKey(), item.getValue().textValue());
        }
        return tempMap;
    }
